...
"""
    ...
    if not shutil.which("pdflatex"):
        st.error("⚠️ 'pdflatex' executable was not found on your system. Please install TeX Live/MiKTeX, or switch to 'ReportLab (Images)' mode.")
        return
...
if not is_cart_empty and st.session_state.get("files_ready"):
    ...
    if os.path.exists(st.session_state.export_paths.get("pdf", "")):
        with open(st.session_state.export_paths["pdf"], "rb") as pdf_file:
            st.sidebar.download_button(
                label="📥 Download Question Paper",
                data=pdf_file,
                file_name=os.path.basename(st.session_state.export_paths["pdf"]),
                mime="application/pdf"
            )
            
    if os.path.exists(st.session_state.export_paths.get("ms", "")):
        with open(st.session_state.export_paths["ms"], "rb") as ms_file:
            st.sidebar.download_button(
                label="📥 Download Mark Scheme",
                data=ms_file,
                file_name=os.path.basename(st.session_state.export_paths["ms"]),
                mime="application/pdf"
            )
